        ...
        <div class="row">
            ...
            <div class="col-lg-9 col-md-9 col-sm-9">
                <div class="card custom-card">
                    <div class="card header custom-card-header text-light">{{ $area->name }} Information</div>
                </div>

                <div class="card-body">
                    <div class="" align="center">
                        <h2 align="center">{{ $area->UnionName->title }}</h2>
                        <address>
                            <p>
                                {{ $area->name }},{{ $area->UnionName->UpazilaName->name }},
                                {{ $area->UnionName->UpazilaName->DistricName->name }} <br>
                                {{ $area->UnionName->phone }} <br>
                                {{ $area->UnionName->email }}
                ...
                <div class="card-body">
                    ...
                    <hr>
                    @if($area->TaxholderInfo->count()>0)
                    <div class="btn-pdf-download mb-2">
                        <a href="{{ route('front.area.pdf',$id) }}" class="btn btn-sm btn-danger"
            ...
            <div class="col-lg-3 col-md-3 col-sm-3">
                ...
                <div class="card-body">
                    ...
                    <ul>
                        @foreach($area->UnionName->AreaDetails as $area)
...
function DataTable() {
    ...
    table = $('#mytable').DataTable({
        ...
        "ajax": {
            "url": "{{ url('Area/TaxHolderLoadAllArea')}}" + '/' + id,
            "type": "GET",
        },
        columns: [{
                data: 'DT_RowIndex',
