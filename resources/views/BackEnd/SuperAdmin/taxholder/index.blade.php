...
function DataTable() {
    table = $('#mytable').DataTable({
        ...
        "ajax": {
            "url": "{{ route('superadmin.taxholder.loadall') }}",
            "type": "GET",
        },
        columns: [{
                data: 'DT_RowIndex',
...
$("#datainsert").on("click", function(e) {
    ...
    if (date == "" && name == "" && holding_number == "" && memo_number == "" && area == "" && amount == "" &&
        amount == "0") {
        $("#overlay").fadeOut();
        $.toast({
            heading: 'Warning',
        ...
        if (taxholderid == 0) {
            ...
            $.ajax({
                type: 'post',
                url: "{{ route('superadmin.taxholder.update') }}",
                data: {
                    id: taxholderid,
                    area_id: area_id,
                    date: date,
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('SuperAdmin/Tax-Holder/delete')}}" + '/' + dataid,
                    success: function(data) {
                        table.ajax.reload();
                        clear();
                    },
...
$(document).on('click', '#datashow', function() {
    ...
    $.ajax({
        type: 'get',
        url: "{{ route('superadmin.taxholder.show') }}",
        data: {
            id: id,
        },
        datatype: 'JSON',
...
function getArea(union_id, area_id) {
    ...
    $.ajax({
        type: 'get',
        url: "{{ url('SuperAdmin/Area/getlist')}}" + '/' + union_id,
        dataType: "JSON",
        success: function(data) {
            $("#area_id").empty();
            $("#area_id").append('<option value="">Select</option>');
